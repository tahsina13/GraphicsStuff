...
The file follows the following format:
     ...
     All the shape commands work as follows:
        ...
        4) Clear the temporary matrix
         ...
         ident: set the peek(csystems) matrix to the identity matrix -
         scale: create a scale matrix,
                then multiply the peek(csystems) matrix by the scale matrix -
                takes 3 arguments (sx, sy, sz)
         move: create a translation matrix,
               then multiply the peek(csystems) matrix by the translation matrix -
               takes 3 arguments (tx, ty, tz)
         rotate: create a rotation matrix,
                 then multiply the peek(csystems) matrix by the rotation matrix -
                 takes 2 arguments (axis, theta) axis should be x y or z
         apply: apply the current peek(csystems)ation matrix to the edge and
                POLYGON matrices
